<script lang="tsx">
    ...
    interface BasicInfo {
        ...
        public_myth_equ?: IEquipmentInfo[]
        public_wisdom_equ?: IEquipmentInfo[]
    }
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            let equi_choose_wisdom = ref<EquiChooseState[]>([])
            ...
            onMounted(async () => {
                ...
                // 神话装备
                basic_info.value.public_wisdom_equ = basicStore?.equipmentinfo?.equipment_wisdom as IEquipmentInfo[]
                // 神话装备记录
                basic_info.value.public_wisdom_equ.forEach(item => {
                    equi_choose_wisdom.value.push({
                        id: item.id,
                        state: false
                    })
                })
                ...
                // 改造装备
                basic_info.value.public_myth_equ = basicStore?.equipmentinfo?.equipment_myth as IEquipmentInfo[]
                // 神话装备记录
                basic_info.value.public_myth_equ.forEach(item => {
            ...
            function selectAll(equList: EquiChooseState[], isL110: boolean = false) {
                return () => {
                    if (choose_feature.value >= 0 && isL110 && equList.filter(item => item.hightlight).length > 0) {
                        let totalState = equList.filter(item => item.state && item.hightlight).length > equList.filter(item => item.hightlight).length / 2
                        equList.forEach(item => {
                            item.state = item.hightlight ? !totalState : item.state
                            item.hightlight = false
                        })
                        choose_feature = ref(-1)
                    } else {
                ...
                <div class="flex">
                    <div class="equ-105">
                        <div class="flex w-100%  mb-5px">
                            <calc-select v-model:modelValue={choose_feature.value} class="ownSelect" emptyLabel="特性选择" onChange={searchEqu}>
                                <calc-option value="-1">全部特性</calc-option>
                                {renderList(featureList, item => (
                                    <calc-option value={item.value}>{item.label}</calc-option>
                                ))}
                            </calc-select>
                            <calc-button class="w-100%" onClick={selectAll(equi_choose_110.value as EquiChooseState[], true)}>
                                105装备
                            </calc-button>
                        </div>
                        <div>
                    ...
                    <div class="equ-else">
                        <div class="equ-else-sort">
                            <calc-button class="w-100% mb-5px" onClick={selectAll(equi_choose_myth.value as EquiChooseState[])}>
                                神话装备
                            </calc-button>
                            <div>
                                {renderList(basic_info.value.public_myth_equ as IEquipmentInfo[], (equ, index) => (
                                    <equip-tips
                                        class="item"
                                        eq={equ}
                                        canClick={true}
                        ...
                        <div class="equ-else-sort">
                            <calc-button class="w-100% mb-5px" onClick={selectAll(equi_choose_wisdom.value as EquiChooseState[])}>
                                智慧产物
                            </calc-button>
                            <div>
                                {renderList(basic_info.value.public_wisdom_equ as IEquipmentInfo[], (equ, index) => (
                                    <equip-tips
                                        class="item"
                                        eq={equ}
                                        canClick={true}
                                        show-tips
                                        active={equi_choose_wisdom.value[index].state}
                                        onChange={changeState(equi_choose_wisdom.value[index])}
                                    ></equip-tips>
                                ))}
                            </div>
                        </div>
                        <div class="equ-else-sort">
                            <calc-button class="w-100% mb-5px" onClick={selectAll(equi_choose_char_weapon.value as EquiChooseState[])}>
                                武器列表
                            </calc-button>
                            ...
                            <div class="m-0">
                                {renderList(basic_info.value.char_weapon_equ as IEquipmentInfo[], (equ, index) => (
                                    <equip-tips
                                        ...
                                        onChange={changeState(equi_choose_char_weapon.value[index])}
                                    ></equip-tips>
                                ))}
                            </div>
                        </div>
                        <div class="equ-else-sort flex-1">
                            <calc-button class="w-100% mb-5px">称号</calc-button>
                        ...
                        <div class="equ-else-sort flex-1">
                            <calc-button class="w-100% mb-5px">宠物</calc-button>
                            <div class="m-0">
                                {renderList(basic_info.value.char_weapon_equ as IEquipmentInfo[], (equ, index) => (
                                    <equip-tips
                                        class="item"
