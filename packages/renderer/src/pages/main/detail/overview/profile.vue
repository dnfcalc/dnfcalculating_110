<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup(props, { emit, slots }) {
      //面板显示的顺序
      const display_parts = ["头肩", "上衣", "下装", "腰带", "鞋", "武器", "称号", "手镯", "项链", "辅助装备", "戒指", "耳环", "魔法石", "宠物"]
      const activeIndex = useVModel(props, "activeIndex", emit)

      function partIconStyle(item: IDetailInfo, cIndex: number = 0) {
        let part = item.position
        let x = 10
        let y = 10
        ...
        <div class="pro-char-info">
          <div class="head" style="background-image:url(images/common/head.png)">
            <div class="w-266px h-170px bg-bottom flex char" style={"background-image:url(images/characters/" + props.charName + "/人物.png);background-repeat: no-repeat; position: absolute;"}>
              ...
              {renderList(props.detailInfo, (item, index) => (
                <div onClick={setPart(index)} class="absolute w-7 h-7" style={partIconStyle(item, index)}></div>
              ))}
            </div>
