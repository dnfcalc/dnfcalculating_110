...
<script lang="tsx">
	...
	import { skills as baseSkills, ISkillInfoForSkillPanel, ISkillTreeForSkillPanel } from './skill-set'
		...
		setup(props, { emit }) {
			...
			const skills = reactive<ISkillTreeForSkillPanel[]>([])
		 	...
		 	async function initSkills() {
				...
				let sks = baseSkills.get(props.character);
				if(sks && sks.length > 0) {
					for(let item of sks) {
						...
						if(tk) {
							...
							// 这里需要修改成一个默认图标
							item.icon = `https://gitee.com/FeelingsCoder/picture-resource-station/raw/master/equipment/arms/swordman/sswd/192.png`
						}
