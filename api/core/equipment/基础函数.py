...
def 基础属性输入(属性):
    ...
    属性.基础力量 = int(角色数据[4] + 角色数据[0] * min(14, 当前等级 - 1) +
                  职业数据[0] * max(当前等级 - 15, 0) + 唤醒 + 2.1 * min(71, 当前等级))
    属性.基础体力 = int(角色数据[5] + 角色数据[1] * min(14, 当前等级 - 1) +
                  职业数据[1] * max(当前等级 - 15, 0) + 唤醒 + 2.0 * min(71, 当前等级))
    属性.基础智力 = int(角色数据[6] + 角色数据[2] * min(14, 当前等级 - 1) +
                  职业数据[2] * max(当前等级 - 15, 0) + 唤醒 + 2.1 * min(71, 当前等级))
    属性.基础精神 = int(角色数据[7] + 角色数据[3] * min(14, 当前等级 - 1) +
...
成长词条系数 = (0, 3, 5, 8, 10, 13, 16, 18, 21, 36, 34, 36, 43, 46, 49, 51, 54, 56, 59, 74, 76, 79, 81, 84, 86, 88, 91, 93, 96, 109, 111, 113, 116,
          118, 121, 123, 125, 128, 130, 143, 145, 148, 150, 152, 154, 156, 159, 161, 163, 174, 176, 178, 179, 181, 183, 185, 187, 188, 190, 199, 201)

# 目前仅有60级内的词条数据
def 成长词条计算(基础, 等级):
    return int(基础 * (1 + 成长词条系数[min(60, 等级 - 1)] / 134.75))
